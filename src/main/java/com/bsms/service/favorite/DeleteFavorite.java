...
@Service("delFavorite1")
public class DeleteFavorite extends MbBaseServiceImpl implements MbService  {
	@Value("${sql.conf}")
    ...
    private static Logger log = LoggerFactory.getLogger(DeleteFavorite.class);
	...
	public MbApiResp process(HttpHeaders header, ContainerRequestContext requestContext, MbApiReq request)
		...
		try (Connection con = DriverManager.getConnection(connectionUrl);) 
        {
			...
			SQL= "delete from Favorite where id_fav='"+request.getId_favorit()+"' and msisdn='"+request.getMsisdn()+"'";
