...
@Slf4j
@Service("doOdcDeactive")
public class OdcDeactive extends MbBaseServiceImpl implements MbService {

    @Value("${cardotp.odc.deActivate}")
    private String odcDeactiveUrl;
    ...
    public MbApiResp process(HttpHeaders header, ContainerRequestContext requestContext, MbApiReq request) throws Exception {
        ...
        log.info("Get CardOtp Odc Deactive Request : " + new Gson().toJson(request));
        ...
        try {
            ...
            String url = odcDeactiveUrl;

            log.info("Get CardOtp Odc Deactive URL : " + url);

            ResponseEntity<BaseResponse> restResponse = restTemps.exchange(url, HttpMethod.POST, req, BaseResponse.class);

            log.info("Get CardOtp Odc Deactive Response : " + new Gson().toJson(restResponse));
