...
@Slf4j
@Service("doOdcUpdate")
public class OdcUpdate extends MbBaseServiceImpl implements MbService {

    @Value("${cardotp.odc.update}")
    private String odcUpdateUrl;
    ...
    public MbApiResp process(HttpHeaders header, ContainerRequestContext requestContext, MbApiReq request) throws Exception {
        ...
        log.info("Get CardOtp Odc Update Request : " + new Gson().toJson(request));
        ...
        try {
            ...
            String url = odcUpdateUrl;

            log.info("Get CardOtp Odc Update URL : " + url);

            ResponseEntity<BaseResponse> restResponse = restTemps.exchange(url, HttpMethod.POST, req, BaseResponse.class);

            log.info("Get CardOtp Odc Update Response : " + new Gson().toJson(restResponse));
