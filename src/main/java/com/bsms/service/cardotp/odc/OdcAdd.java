...
@Slf4j
@Service("doOdcAdd")
public class OdcAdd extends MbBaseServiceImpl implements MbService {

    @Value("${cardotp.odc.add}")
    private String odcAddUrl;
    ...
    public MbApiResp process(HttpHeaders header, ContainerRequestContext requestContext, MbApiReq request) throws Exception {
        ...
        log.info("Get CardOtp Odc Add Request : " + new Gson().toJson(request));
        ...
        try {
            ...
            String url = odcAddUrl;

            log.info("Get CardOtp Odc Add URL : " + url);

            ResponseEntity<BaseResponse> restResponse = restTemps.exchange(url, HttpMethod.POST, req, BaseResponse.class);

            log.info("Get CardOtp Odc Add Response : " + new Gson().toJson(restResponse));
