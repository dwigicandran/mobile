...
@Service("egoldInfo")
public class EgoldInfoServiceImpl extends MbBaseServiceImpl implements MbService {

    private static Logger log = LoggerFactory.getLogger(EgoldInfoServiceImpl.class);

    @Value("${emas.dashboard.info}") private String endpoint;
    @Value("${emas.checkaccount.info}") private String endpointCheckAccount;
    ...
    public MbApiResp process(HttpHeaders header, ContainerRequestContext requestContext, MbApiReq request) throws Exception {
        String url;
        if("1".equals(request.getFlag())){ // dashboard
            url = endpoint;
        } else {
            url = endpointCheckAccount;
        }
