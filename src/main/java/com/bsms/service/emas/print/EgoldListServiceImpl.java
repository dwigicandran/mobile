package com.bsms.service.emas.print;
...
@Service("printEgoldList")
public class EgoldListServiceImpl extends MbBaseServiceImpl implements MbService {

    private static Logger log = LoggerFactory.getLogger(EgoldListServiceImpl.class);

    @Value("${emas.list.allowedbranch}") private String urlAllowedBranch;
    @Value("${emas.list.type}") private String urlListType;
    @Value("${emas.list.calculateprint}") private String urlCalculatePrint;
    @Value("${emas.list.history}") private String urlHistoryList;
    @Value("${emas.list.verify}") private String urlVerifyList;
    @Value("${rest.template.timeout}") private int restTimeout;

    @Autowired private MbAppContentRepository mbAppContentRepository;
    ...
    public MbApiResp process(HttpHeaders header, ContainerRequestContext requestContext, MbApiReq request) throws Exception {
        ...
        if("allowedbranch".equals(requestParam)) {
            url = urlAllowedBranch;
        } else if("listoftype".equals(requestParam)) {
            url = urlListType;
        } else if("calculateprint".equals(requestParam)){
            url = urlCalculatePrint;
        } else if("history".equals(requestParam)){
            url = urlHistoryList;
        } else if("verify".equals(requestParam)){
            url = urlVerifyList;
        }
